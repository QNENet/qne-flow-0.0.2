...
public class WebJarServer implements Serializable {
	...
	/**
	 * Creates a webJar server that is able to search webJars for files and return
	 * them.
	 *
	 * @param deploymentConfiguration configuration for the deployment, not
	 *                                <code>null</code>
	 *
	 */
	public WebJarServer(DeploymentConfiguration deploymentConfiguration) {
		...
		if (!frontendPrefix.startsWith(ApplicationConstants.CONTEXT_PROTOCOL_PREFIX)) {
			throw new IllegalArgumentException(
					"Cannot host WebJars for a fronted prefix that isn't relative to 'context://'. Current frontend prefix: "
							+ frontendPrefix);
		}

		prefix = "/" + frontendPrefix.substring(ApplicationConstants.CONTEXT_PROTOCOL_PREFIX.length())
				+ "bower_components/";
		urlPattern = Pattern.compile("^([/.]?[/..]*)" + prefix);
	}

	/**
	 * Searches for file requested in the webJars. If found, the file contents is
	 * written into request.
	 *
	...
	public boolean tryServeWebJarResource(HttpServletRequest request, HttpServletResponse response) throws IOException {
		String pathInContext = request.getPathInfo() == null ? request.getServletPath()
	...
	public Optional<String> getWebJarResourcePath(String filePathInContext) {
		...
		if (matcher.find()) {
			webJarPath = getWebJarPath(filePathInContext.substring(matcher.group(1).length()));
		}
