...
public class RouteTarget implements Serializable {
	 ...
	 *
	 * @throws InvalidRouteConfigurationException exception for miss configured
	 *                                            routes where navigation targets
	 ...
	 * <p>
	 * When adding a new target it will be validated that it is a valid path to add
	 * with the already existing navigation targets.
	 *
	 * @param target navigation target to add
	 * @throws InvalidRouteConfigurationException exception for miss configured
	 *                                            routes where navigation targets
	 *                                            can not be clearly selected
	 */
	public void addRoute(Class<? extends Component> target) throws InvalidRouteConfigurationException {
		if (!HasUrlParameter.class.isAssignableFrom(target) && !isAnnotatedParameter(target)) {
	...
	private void validateNormalTarget(Class<? extends Component> target) throws InvalidRouteConfigurationException {
		if (normal != null) {
			...
			throw new InvalidRouteConfigurationException(String.format(
					"Navigation targets '%s' and '%s' have the same path and '%s' has an OptionalParameter that will never be used as optional.",
					target.getName(), optionalParameter.getName(), optionalParameter.getName()));
		}
	}
	...
	private boolean isAnnotatedParameter(Class<?> target) {
		return ParameterDeserializer.isAnnotatedParameter(target, OptionalParameter.class)
				|| ParameterDeserializer.isAnnotatedParameter(target, WildcardParameter.class);
	}
	 ...
	 * 
	 * @param target navigation target
	 * @param theme  themeDefinition to be used when the navigation target is used
	 */
