...
public class RouteRegistry implements Serializable {
	...
	public static class ErrorTargetEntry implements Serializable {
		...
		/**
		 * Creates a new new entry with the given navigation target type and exception
		 * type.
		 *
		 * @param navigationTarget     the navigation target type, not <code>null</code>
		 * @param handledExceptionType the exception type handled by the navigation
	...
	private static class OSGiRouteRegistry extends RouteRegistry {
		...
		private void initErrorTargets() {
			...
			OSGiDataCollector registry = (OSGiDataCollector) getInstance(osgiServletContext);
		...
		private void doInitOSGiRoutes() throws InvalidRouteConfigurationException {
			if (navigationTargetsInitialized() || OSGiAccess.getInstance().getOsgiServletContext() == null) {
				return;
			}
	...
	private static class OSGiDataCollector extends RouteRegistry {
		...
		@Override
		public void setNavigationTargets(Set<Class<? extends Component>> navigationTargets)
				throws InvalidRouteConfigurationException {
			if (navigationTargets.isEmpty() && this.navigationTargets.get() == null) {
	...
	private static final Set<Class<? extends Component>> defaultErrorHandlers = Stream
			.of(RouteNotFoundError.class, InternalServerError.class).collect(Collectors.toSet());
	 ...
	 *
	 * @return the Lumo ThemeDefinition, or <code>null</code> if it is not available
	 *         in the classpath
	 */
	private static final ThemeDefinition loadLumoClassIfAvailable() {
		try {
			...
			logger.trace("Lumo theme is not present in the classpath. The application will not use any default theme.",
					e);
		}
		return null;
	}
	...
	/**
	 * Gets the route registry for the given servlet context. If the servlet context
	 * has no route registry, a new instance is created and assigned to the context.
	 *
	 * @param servletContext the servlet context for which to get a route registry,
	 *                       not <code>null</code>
	 *
	 ...
	 *
	 * @param navigationTargets set of navigation target components
	 * @throws InvalidRouteConfigurationException if routing has been configured
	 *                                            incorrectly
	 */
	public void setNavigationTargets(Set<Class<? extends Component>> navigationTargets)
			throws InvalidRouteConfigurationException {
	 ...
	 *
	 * @param errorNavigationTargets error handler navigation targets
	 */
	public void setErrorNavigationTargets(Set<Class<? extends Component>> errorNavigationTargets) {
		...
		for (Class<? extends Component> target : errorNavigationTargets) {
			if (defaultErrorHandlers.contains(target)
					|| !routeFilters.stream().allMatch(filter -> filter.testErrorNavigationTarget(target))) {
				continue;
			}

			Class<? extends Exception> exceptionType = ReflectTools
					.getGenericInterfaceType(target, HasErrorParameter.class).asSubclass(Exception.class);
	...
	public List<RouteData> getRegisteredRoutes() {
		...
		Map<Class<? extends Component>, String> targetRouteMap = targetRoutes.get();
	 ...
	 * allowed.
	 *
	 * @param target        target being handled
	 * @param exceptionType type of the handled exception
	 */
	...
	/**
	 * Get a registered navigation target for given exception. First we will search
	 * for a matching cause for in the exception chain and if no match found search
	 * by extended type.
	 *
	 * @param exception exception to search error view for
	...
	private ErrorTargetEntry searchByCause(Exception exception) {
		Class<? extends Component> targetClass = exceptionTargets.get().get(exception.getClass());
	...
	private ErrorTargetEntry searchBySuperType(Throwable exception) {
		...
		while (superClass != null && Exception.class.isAssignableFrom(superClass)) {
			Class<? extends Component> targetClass = exceptionTargets.get().get(superClass);
			if (targetClass != null) {
	...
	/**
	 * Gets the optional navigation target class for a given Location. Returns an
	 * empty optional if no navigation target corresponds to the given Location.
	 *
	 * @see Location
	 *
	 * @param pathString the path to get the navigation target for, not {@code null}
	 * @return optional of the navigation target corresponding to the given location
	 */
	public Optional<Class<? extends Component>> getNavigationTarget(String pathString) {
	...
	/**
	 * Gets the optional navigation target class for a given Location matching with
	 * path segments.
	 *
	 *
	 * @see Location
	 *
	 * @param pathString path to get navigation target for, not {@code null}
	 * @param segments   segments given for path
	 * @return optional navigation target corresponding to the given location with
	 *         given segments if any applicable targets found.
	 */
	public Optional<Class<? extends Component>> getNavigationTarget(String pathString, List<String> segments) {
		if (hasRouteTo(pathString)) {
			return Optional.ofNullable(getRoutes().get(pathString).getTarget(segments));
		}
	 ...
	 *
	 * @param navigationTarget navigation target to get registered route for, not
	 *                         {@code null}
	 ...
	 *
	 * @param navigationTarget navigation target to generate url for
	 * @return route with required parameters
	 */
	private String collectRequiredParameters(Class<? extends Component> navigationTarget) {
		StringBuilder route = new StringBuilder(targetRoutes.get().get(navigationTarget));
	 ...
	 *
	 * @param navigationTarget navigation target to get chain route for
	...
	private Collection<String> getRouteAliases(Class<?> navigationTarget) {
		...
		for (RouteAlias alias : navigationTarget.getAnnotationsByType(RouteAlias.class)) {
			aliases.add(RouterUtil.getRouteAliasPath(navigationTarget, alias));
		}
		return aliases;
	}

	private void registerNavigationTargets(Set<Class<? extends Component>> navigationTargets)
			throws InvalidRouteConfigurationException {
		...
		for (Class<? extends Component> navigationTarget : navigationTargets) {
			if (!navigationTarget.isAnnotationPresent(Route.class)) {
				throw new InvalidRouteConfigurationException(String.format(
						"No Route annotation is present for the given " + "navigation target component '%s'.",
		...
		if (!routes.compareAndSet(null, Collections.unmodifiableMap(routesMap))) {
			handleInitializedRegistry();
		}
		if (!targetRoutes.compareAndSet(null, Collections.unmodifiableMap(targetRoutesMap))) {
			handleInitializedRegistry();
		}
	}

	private void addRoute(Map<String, RouteTarget> routesMap, Class<? extends Component> navigationTarget,
			Collection<String> paths) throws InvalidRouteConfigurationException {
		Logger logger = LoggerFactory.getLogger(RouteRegistry.class.getName());
		for (String path : paths) {
			...
			if (routesMap.containsKey(path)) {
				...
				logger.debug("Registering route '{}' to navigation target '{}'.", path, navigationTarget.getName());

				routeTarget = new RouteTarget(navigationTarget);
				routesMap.put(path, routeTarget);
			}
			routeTarget.setThemeFor(navigationTarget, findThemeForNavigationTarget(navigationTarget, path));
		}
	}
	...
	private ThemeDefinition findThemeForNavigationTarget(Class<?> navigationTarget, String path) {
		...
		Class<?> target = topParentLayout == null ? navigationTarget : topParentLayout;
	 ...
	 *
	 * @return <code>true</code> if at least one navigation target is registered;
	...
	/**
	 * Gets the {@link ThemeDefinition} associated with the given navigation target,
	 * if any. The theme is defined by using the {@link Theme} annotation on the
	 * navigation target class.
	 * <p>
	 * If no {@link Theme} and {@link NoTheme} annotation are used, by default the
	 * {@code com.vaadin.flow.theme.lumo.Lumo} class is used (if present on the
	 * classpath).
	 *
	 * @param navigationTarget the navigation target class
	 * @param path             the resolved route path so we can determine what the
	 *                         rendered target is for
	 * @return the associated ThemeDefinition, or empty if none is defined and the
	 *         Lumo class is not in the classpath, or if the NoTheme annotation is
	 *         being used.
	 */
	public Optional<ThemeDefinition> getThemeFor(Class<?> navigationTarget, String path) {
		...
		if (navigationTarget != null && navigationTargetsInitialized()) {
			...
			Map<Class<? extends Component>, String> targetRoutesMap = targetRoutes.get();
			...
			if (routeTarget != null) {
				ThemeDefinition theme = routeTarget.getThemeFor(navigationTarget);
				if (theme != null) {
					return Optional.of(theme);
				}
			}
		}
		return Optional.ofNullable(findThemeForNavigationTarget(navigationTarget, path));
	}
	...
	private static RouteRegistry createRegistry(ServletContext context) {
		if (context != null && context == OSGiAccess.getInstance().getOsgiServletContext()) {
